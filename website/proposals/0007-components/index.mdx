...
This document proposes an enhancement to Magek Framework, introducing a new component integration mechanism. This enhancement aims not to overhaul the existing architecture but to provide an incremental upgrade, allowing the Framework to retain its current functionality while adding new, more efficient capabilities. This proposal details the transition to a system where components, written with clear interfaces using [Effect](https://www.effect.website/), can be directly injected into the Magek project, offering improved efficiency and customization.


:::warning

This component system is **not meant to be used by all Magek users**, but rather by it's **power users** who want to extend their apps in ways that are not possible with the current system, as well as **Magek contributors** who want to add new features to the framework.

:::

## Current Mechanism and Its Limitations

The Magek Framework currently relies on users specifying packages to be loaded (like Azure provider or filesystem components) in a JSON configuration, which are then **dynamically imported** by the framework. While functional, this approach has limitations in efficiency and customization.
...
- **Inversion of Control**: Shift from dynamic imports to a system where users inject components directly into their Magek projects.
...
- **Backward Compatibility**: Ensures existing Magek applications continue to function without mandatory modifications.
...
This is an extremely powerful idea, because with a little subset of primitive components, we allow extending and customizing Magek apps in
...
These components would get injected into the `Magek.config` block for now, as it gets loaded when
the application starts.

If a Magek application is configured with some components, it's main executable will get extended with an additional CLI flag `--exec` that
...
This way of working allows taking responsibility out of the Magek CLI tool and allow users to create a regular `boost` (or any more descriptive name)
...
- **Compatibility Measures**: Ensure the new component system aligns with the existing Magek Framework.
...
This proposal lays the groundwork for a progressive enhancement in Magek Framework's component integration mechanism. This proposal balances the need for advanced functionality and efficiency with the necessity of maintaining existing systems, while giving power users a great extensibility tool, and
