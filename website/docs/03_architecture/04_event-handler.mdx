...
```shell
npx boost new:event-handler HandleAvailability --event StockMoved
```
...
Event handlers can be easily created using the Booster CLI command `npx boost new:event-handler`. There are two mandatory arguments: the event handler name, and the name of the event it will react to. For instance:
...
**Booster** includes a `Global event handler`. This feature allows you to react to any event that occurs within the system.
