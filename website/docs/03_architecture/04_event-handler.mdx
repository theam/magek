...
Event handlers can be easily created using the Magek CLI command `npx boost new:event-handler`. There are two mandatory arguments: the event handler name, and the name of the event it will react to. For instance:
