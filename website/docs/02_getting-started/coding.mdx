...
Before starting, make sure you have [Node.js installed](/getting-started/installation). You'll need the Booster CLI later for development tasks, but project creation doesn't require any global installations.
...
```shell
npm create booster-ai@latest boosted-blog
```

</TerminalWindow>

The create script will prompt you for project details like description, author, and license. You can press Enter to accept the defaults for any field. The script automatically configures your project to use **Booster Server**, which allows you to run and debug your application locally.
...
:::tip
You can create a project non-interactively by providing command line options:

```shell
npm create booster-ai@latest my-app -- --author "Your Name" --description "My awesome Booster app"
```
...
> - `booster-ai@latest` specifies the package name and version to use for creating projects
...
:::tip
You can also use equivalent commands with other package managers:
- `npx create-booster-ai@latest boosted-blog`  
- `pnpm dlx create-booster-ai@latest boosted-blog`
- `bun create booster-ai boosted-blog`
:::
...
Now that you have a project, navigate to your project directory and set up your development environment. The project includes the Booster CLI as a dependency, so you can use it immediately:
...
```bash
npx boost new:command CreatePost --fields postId:UUID title:string content:string author:string
```
...
```bash
npx boost new:event PostCreated --fields postId:UUID title:string content:string author:string
```
...
```bash
npx boost new:entity Post --fields title:string content:string author:string --reduces PostCreated
```
...
<TerminalWindow>
```bash
npx boost new:read-model PostReadModel --fields title:string author:string --projects Post:id
```
</TerminalWindow>
...
```bash
npx boost start -e local -p 8080
```
...
we ran the `npm create booster-ai` command.

<TerminalWindow>

```bash
> npx boost nuke -e production
