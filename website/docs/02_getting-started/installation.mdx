...
### Installing the Booster CLI (Optional)

For project development after creation, Booster comes with a command-line tool that helps you generate boilerplate code, test and deploy the application, and delete all the resources in the cloud. All the stable versions are published to <CLInstallBooster>[`npm`](https://www.npmjs.com/package/@booster-ai/cli)</CLInstallBooster>, these versions are the recommended ones, as they are well documented, and the changes are stated in the release notes.
...
> @booster-ai/cli/0.16.1 darwin-x64 node-v14.14.0

</TerminalWindow>

:::tip
The CLI is only needed for development tasks after project creation (generating commands, entities, etc.). For project creation, use `npm create booster-ai@latest` instead.
:::
