...
*Infrastructure Rocket* interfaces are provider-dependent because each provider defines their own way to manage context, so *Infrastructure Rockets* must import the corresponding magek infrastructure package for their chosen provider:
...
- **mountStack**: Whenever we are deploying our Magek application (`npx boost deploy`) this method will also be run.  It receives two params:
...
```sh
$ npm install my-magek-rocket
```

```typescript
// src/MySpecialClass.ts
import { MyCustomDecorator, RocketSingleton } from 'my-magek-rocket';
