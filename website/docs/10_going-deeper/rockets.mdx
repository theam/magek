...
All providers use the same package: `@booster-ai/server-infrastructure`.
...
So let's start by creating a new package and adding the provider-dependent dependency as well as the `typescript` and `@booster-ai/common` packages:
...
npm init
...
npm install --save-dev @booster-ai/server-infrastructure @booster-ai/common typescript
```
...
```typescript
import { RocketUtils } from '@booster-ai/server-infrastructure'
...
- **mountStack**: Whenever we are deploying our Booster application (`npx boost deploy`) this method will also be run.  It receives two params:
...
```typescript
import { InfrastructureRocket } from '@booster-ai/server-infrastructure'
