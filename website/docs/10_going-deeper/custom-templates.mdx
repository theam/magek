...
<details>
    ...
    Project creation now uses the modern `npm create` pattern instead of the CLI. You can use custom project templates by specifying a GitHub repository:

    ```bash
    npm create booster-ai@latest my-app -- --template user/custom-booster-template
    ```

    The template repository should contain a Booster project structure with mustache placeholders like `{{PROJECT_NAME}}`, `{{description}}`, etc.
</details>
