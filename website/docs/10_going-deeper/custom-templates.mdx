...
To do this, you first need to publish the resource templates by running the `npx boost stub:publish` command. This will create a folder `stubs` in the root directory of the project, and it will contain all the resources that you can customize:
...
Now if you run `npx boost new:type CartItem --fields sku:string` command, you will get `common/cart-item.ts` file with following content:
```typescript
// Look I am a comment that will now appear in every new type file 🐙
...
You did it, we just updated our resource template file! Now when you run `npx boost new:type`, it will contain the comment you added earlier 🚀
...
<details>
    ...
    1. Run `npx boost stub:publish --force` command
    2. Review changes
    3. Done!
</details>
...
<details>
    <summary>How do I use custom project templates?</summary>
    ...
    **From GitHub repositories:**
    ```bash
    npm create booster-ai@latest my-app -- --template user/custom-booster-template
    ```

    **From local filesystem:**
    ```bash
    npm create booster-ai@latest my-app -- --template ./path/to/local/template
    ```

    The template should contain a Booster project structure with mustache placeholders like `{{PROJECT_NAME}}`, `{{description}}`, etc.
</details>
