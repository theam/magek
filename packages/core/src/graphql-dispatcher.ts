...
export class MagekGraphQLDispatcher {
  ...
  public constructor(private config: MagekConfig) {
    ...
    // Create a simple adapter to bridge the connection interface
    const connectionAdapter = {
      storeData: (config: MagekConfig, connectionID: string, data: any) =>
        this.config.sessionStore.storeConnection(config, connectionID, data),
      fetchData: (config: MagekConfig, connectionID: string): Promise<any> =>
        this.config.sessionStore.fetchConnection(config, connectionID),
      deleteData: (config: MagekConfig, connectionID: string) =>
        this.config.sessionStore.deleteConnection(config, connectionID),
      sendMessage: (config: MagekConfig, connectionID: string, data: unknown) => {
        // TODO: sendMessage functionality needs to be implemented separately
        // from session storage, possibly through a separate message adapter
