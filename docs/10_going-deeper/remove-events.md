...
Magek allows to delete past events and their related entities as to update the affected ReadModels.

By using the `Magek.deleteEvent` command it is possible to indicate the event to be deleted. To do so, you must indicate:
...
Example:
```typescript
import { Magek, Command } from '@magek/core'
import { EventDeleteParameters } from '@magek/common'
...
export class HardDelete {
  ...
  public static async handle(command: HardDelete): Promise<boolean> {
    ...
    return await Magek.deleteEvent(parameters)
  }
}
```

When executing this command, Magek will update the selected event in the corresponding database with an empty value and a deletion date.
This way, it will be reflected in the system that there was an event that was subsequently deleted.

Deleted events are ignored by Magek, but they can be accessed using the corresponding methods (`eventsByEntity` and `eventsByType`).
...
([See more details about how to delete a ReadModel in the docs](https://docs.magek.ai/architecture/read-model/#deleting-read-models))
