# Extending Magek with Rockets!

You can extend Magek by creating rockets (Magek Framework extensions). A rocket is just a node package that implements the public Magek rocket interfaces. You can use them for:
...
If you want to create a rocket that supports several cloud providers or want to provide extra decorators and functionality on top of the infrastructure extensions, you'll probably need to distribute it as a set of separate packages. In this scenario we recommend using a monorepo management tool like [Microsoft Rush](https://rushjs.io) to maintain them all together in a single repository, but this is not a requirement. Your packages will work perfectly fine if you maintain them in separate repositories.

### Create an Infrastructure Rocket package to extend the default Magek-provided infrastructure
...
So let's start by creating a new package and adding the provider-dependent dependency as well as the `typescript` and `@magek/common` packages:
...
export class YourMainClass {
  public static mountStack(params: YourRocketParams, stack: Stack, config: MagekConfig): void {
    /* CDK code to expand your Magek infrastructure */
  }
...
- **mountStack**: Whenever we are deploying our Magek application (`npx magek deploy`) this method will also be run.  It receives two params:
  - `stack`: An initialized AWS CDK stack that you can use to add new resources. Check out [the Stack API in the official CDK documentation](https://docs.aws.amazon.com/cdk/latest/guide/stacks.html#stack_api). This is the same stack instance that Magek uses to deploy its resources, so your resources will automatically be deployed along with the Magek's ones on the same stack.
  - `config`: It includes properties of the Magek project (e.g. project name) that come in handy for your rocket.

- **unmountStack**: This function executes when you run the `npx magek nuke` command, just before starting the deletion of the cloud resources. When you nuke your Magek application, resources added by your rocket are automatically destroyed with the rest of the application stack. However, in certain cases, you may need extra steps during the deletion process. The `unmountStack` function serves this purpose. For example, in AWS, you must first empty any S3 buckets before deleting your stack. You can achieve this within the `unmountStack` method.
...
export interface InfrastructureRocket {
  mountStack: (stack: Stack, config: MagekConfig) => void
...
Your rocket implementation will have access to the stack (CDK in AWS or Terraform in Azure) just after Magek has finished to add all its default resources, so while the most common scenario to implement a rocket is to create additional resources, it's also possible to inspect or alter the Magek stack. If you're considering creating and maintaining your own fork of one of the default provider runtime implementations, it could be easier to create a rocket instead.

### Provide new abtractions with custom decorators

Rockets can be utilized to extend the Magek framework by providing additional decorators that offer new abstractions. When creating a decorator as part of your rocket, you should deliver it as a package that, once compiled, does not have any infrastructure dependencies, so if your rocket provides both infrastructure and runtime extensions, it's advisable to deliver it as a pair of packages or more.

A common pattern when creating decorators for Magek is to use a singleton object to store metadata about the decorated structures. This singleton object stores data generated during the decorator's execution, which can then be accessed from other parts of the user's project, the rocket's infrastructure package or even other rockets. This data can be used during deployment to generate extra tables, endpoints, or other resources.

To create a new custom decorator for the Magek framework with singleton storage, follow these steps:
...
Now you have a custom decorator that can be used within the Magek framework. Users can install your rocket package and use the decorator in their Magek applications:

```sh
$ npm install my-rocket
```

```typescript
// src/MySpecialClass.ts
import { MyCustomDecorator, RocketSingleton } from 'my-rocket';
...
This example demonstrates how to create a custom decorator with a singleton object for storing data and package it as a rocket for use with the Magek framework. Following this pattern will allow you to extend Magek with new abstractions and provide additional functionality for users. The singleton object can be used to store and retrieve data across different parts of the user's project, enabling features such as generating extra tables or endpoints during deployment. This approach ensures a consistent and flexible way to extend the Magek framework while maintaining ease of use for developers.
...
Here you can check out the official Magek Rockets developed at this time:

## Available Rockets

- [Rocket Backup Booster](rockets/rocket-backup-booster.md)
- [Rocket File Uploads](rockets/rocket-file-uploads.md)
- [Rocket Static Sites](rockets/rocket-static-sites.md)
- [Rocket Webhook](rockets/rocket-webhook.md)