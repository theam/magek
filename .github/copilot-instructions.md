...
1. **Start in `packages/core/src`** – Contains the core domain logic (commands, events, GraphQL generation, etc.).
2. **Server runtime** lives in `packages/server/src`.
3. **Shared utilities** (like `evolve()`) are in `packages/common/src`.
4. **CLI entry point** is `packages/cli/bin/run` (OCLIF-based CLI).
5. **Storage adapters** are in `adapters/nedb/` for local development.
